...
def get_args():
    ...
    parser.add_argument("--tag_mission", type=str, default="quality", help="The tagging mission.", choices=["difficulty", "quality", "classification", "safety", "reward", "language", "token_count","refined","all"])
    parser.add_argument("--model_path", type=str, default="meta-llama/Meta-Llama-3-8B-Instruct", help="Tag Model.")
    parser.add_argument("--guard_model_path", type=str, default="meta-llama/Llama-Guard-3-8B", help="Guard Model.")
    parser.add_argument("--reward_model_path", type=str, default="internlm/internlm2-7b-reward", help="Reward Model.")
    ...
    parser.add_argument("--save_as", type=str, default="jsonl", choices=["jsonl"], help="Save the generated responses as a what kind of file")
...
def generate_tags(device, model, tag_mission, max_tokens, temperature, repetition_penalty, input_file, batch_size, save_as, gpu_memory_utilization=0.95):
    print('Generating {tag_mission} tags...'.format(tag_mission=tag_mission))

    llm = gen_llm(device, model, tag_mission, max_tokens, temperature, repetition_penalty, gpu_memory_utilization)
    llm.load()

    with open(f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_{tag_mission}.{save_as}", 'a+', encoding='utf8') as out:
        ...
        for i in tqdm(range(0, len(data_list), batch_size)):
            ...
            for batch in all_batch:
                if "system" in batch['conversations'][0]['from']:
                    after_batch.append([
                        {"role": "user", "content": input_rating(batch['conversations'][1]['value'])},
                    ])   
                else:
                    after_batch.append([
                        {"role": "user", "content": input_rating(batch['conversations'][0]['value'])},
                    ])
...
def token_count(input_file, model):
    ...
    with open(f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_token_count.jsonl", 'a+', encoding='utf8') as out:
        data_list = load_jsonl_to_list(input_file)
    ...
    output_file = f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_token_count.jsonl"
...
def language_detection_turns(input_file):
    ...
    with open(f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_language_turns.jsonl", 'a+', encoding='utf8') as out:
        data_list = load_jsonl_to_list(input_file)
        for i, n in tqdm(enumerate(data_list)):
            text = ''
    ...
    output_file = f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_language_turns.jsonl"
    return output_file
...
def safety_tag(model, device, input_file, batch_size, save_as):
    ...
    safety_llm = LLM(model=model, gpu_memory_utilization=0.95, tensor_parallel_size=tensor_parallel_size, enable_prefix_caching=True, max_model_len=4096)
    safety_tokenizer = AutoTokenizer.from_pretrained(model)

    with open(f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_safety.{save_as}", 'a+', encoding='utf8') as out:
        ...
        for i in tqdm(range(0, len(data_list), batch_size)):
            ...
            for item in all_batch:
                text_safety = safety_tokenizer.apply_chat_template(conversations_mapping(item['conversations']), tokenize=False)
    ...
    output_file = f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_safety.{save_as}"
    destroy_model_parallel()
...
def refined_result(input_file, save_as):
    with open(f"{input_file.split(os.path.splitext(input_file)[-1])[0]}_refined.{save_as}", 'a+', encoding='utf8') as out:
        with open(input_file, 'r', encoding='utf8') as file:
            key_list = ['id', 'conversations', 'difficulty', 'classification', 'quality', 'safety', 'reward', 'language', 'turns', 'token_count', 'source']
            for line in jsonlines.Reader(file):
                refined = {}
                for key in key_list:
                    if key in line:
...
def main():
    if args.tag_mission in ["difficulty", "classification", "quality"]:
        generate_tags(args.device, args.model_path, args.tag_mission, args.max_tokens, args.temperature, args.repetition_penalty, args.input_file, args.batch_size, args.save_as)
    elif args.tag_mission == "language":
        language_detection_turns(args.input_file)
    elif args.tag_mission == "safety":
        safety_tag(args.guard_model_path, args.device, args.input_file, args.batch_size, args.save_as)
    elif args.tag_mission == "reward":
        reward_tag(args.reward_model_path, args.device, args.input_file, args.batch_size, args.save_as)
